...
<%          int current = 0; 
            ...
            ResultSet rs = null;
%>
	<FORM NAME="form1" ACTION="League.jsp" METHOD="get"> 
	<%
		            ...
		            try {
		                ...
		                rs = statement.executeQuery("select r.lg_id,r.name, r.played,r.goalsfor,r.goalsagainst,"+
		                		"r.goalsdifference,r.win,r.draw,r.loss,r.total "+
		                		"from ranking r "+
		                	    "INNER JOIN leagues l "+
		                	     "ON l.lg_id=r.lg_id "+
		                	    " INNER JOIN sport s "+
		                	     "ON s.sprt_name = l.sprt_id "+
		                	    "WHERE s.sprt_id= "+"'soc1'"+";");
		                if (rs.next()) {
    ...
    <table class="fl-table">
        ...
        <%while(rs.next())
        {
            ...
            <tr>
                ...
                <td><%=rs.getInt("goalsfor") %></td>
                <td><%=rs.getInt("goalsagainst") %></td>
                <td><%=rs.getInt("goalsdifference") %></td>
                <td><%=rs.getInt("win") %></td>
